 ...
 */
function ChallengeError(url: string) {
...
export default class APIHelper {
    ...
    always(batch: pogobuf.Client, options?: any): pogobuf.Client {
        ...
        if (!options.noquest) batch.getNewQuests();
        ...
        if (!last || last < this.state.api.item_templates_timestamp) {
            ...
            for (const template of item_templates) {
                for (const name in template) {
                    if (!template.hasOwnProperty(name)) continue;
                    if (!template[name]) delete template[name];
                }
            }
            const json = JSON.stringify({
